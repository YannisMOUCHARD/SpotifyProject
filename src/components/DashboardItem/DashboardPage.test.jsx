...
describe('DashboardPage', () => {
    ...
    test('redirects to login on token expiration', async () => {
        ...
        // Ajouter ces tests à la fin du describe block
    
    test('does not fetch data when no token is available', async () => {
        ...
        expect(spotifyApi.fetchUserTopArtists).not.toHaveBeenCalled();
        expect(spotifyApi.fetchUserTopTracks).not.toHaveBeenCalled();
    
        // Should show no data messages
        expect(screen.getByText('No top artist data available')).toBeInTheDocument();
        expect(screen.getByText('No top track data available')).toBeInTheDocument();
    });
    
    test('displays no data when empty response is returned', async () => {
        // Mock empty response
